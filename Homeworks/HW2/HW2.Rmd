...
I would expect strong correlation among members of the same party, meaning that about 50\% of the senate will be correlated together. There are also a few members of both parties that will likely be on the extremes of that party and not that correlated. 
...
swapped_mitch_negativity <- data.frame(t(negative_with_mitch[-1]))
colnames(swapped_mitch_negativity) <- negative_with_mitch[,1]


swapped_mitch_negativity
...
```{r}
senate <- as_tibble(rbind(voters_matrix, votes_matrix), rownames = NA)
head(senate)
```
