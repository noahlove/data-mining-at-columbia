...
convertJSON = function(jsonFile, OutputFile){
  
  
  json_data <- fromJSON(paste(readLines(jsonFile)))
  
  output = ldply(json_data, data.frame)
  write.csv(x=output, file=OutputFile)
}

convertJSON('votes_small.json', 'output1.csv')
...
voting_matrix <- plyr::rbind.fill(as.data.frame(votes))
```
...
#### Example of the first part of the data to take out
